...
public class EmployeeController {
    ...
    @ApiOperation("新增员工")
    @PostMapping
    public Result newlyIncreased(@RequestBody EmployeeDTO employeeDTO){
        //调用service保存
        employeeService.newlyIncreased(employeeDTO);
        return Result.success();
    }

    /**
     * 分页查询
     * @param employeePageQueryDTO 接收姓名,当前页,每页个数
     * @return
     */
    @ApiOperation("分页查询")
    @GetMapping("/page")
    ...
    public Result state(@PathVariable Integer status ,Long id){
        //调用service修改
        employeeService.state(status,id);
        return Result.success();
    }
